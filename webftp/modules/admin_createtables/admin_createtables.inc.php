...
// Connect
// ------------------------------------
		$mydb2 = mysqli_connect($dbserver2, $dbusername2, $dbpassword2);
		if ($mydb2 == false) { $net2ftp_output["admin_createtables"][] = __("Unable to connect to the server <b>%1\$s</b>.", $dbserver2_html) . "\n"; }

// ------------------------------------
// Select
// ------------------------------------
		$result2 = mysqli_select_db($mydb2, $dbname2);
		if ($result2 == false) { $net2ftp_output["admin_createtables"][] = __("Unable to select the database <b>%1\$s</b>.", $dbname2_html) . "\n"; }
...
// ------------------------------------
		if ($result2 != false) {
			for ($i=0; $i<sizeof($sqlquerypieces); $i++) {
				$mysqli_query_results[$i] = mysqli_query($mydb2, $sqlquerypieces[$i]);
				if ($mysqli_query_results[$i] == false) { $net2ftp_output["admin_createtables"][] = __("The SQL query nr <b>%1\$s</b> could not be executed.", $i+1) . "\n"; }
