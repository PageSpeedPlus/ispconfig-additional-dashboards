...
// Open the first (main) or second connection
	if ($connection_number == 1) {
		...
		if (isset($_SESSION["net2ftp_password_encrypted_" . $ftpserver . $username]) == true) { $password = decryptPassword($_SESSION["net2ftp_password_encrypted_" . $ftpserver . $username]); }
		else                                                                                  { $password = decryptPassword($net2ftp_globals["password_encrypted"]); }
	}
	else {
...
function ftp_rename3($conn_id, $old, $new) {
...
// Take Windows shared directory into account \\server\web\mysite.com\net2ftp
	if (substr($part1, 0, 2) == "\\\\") { $part1 = "\\\\" . stripDirectory($part1); }
	else { $part1 = stripDirectory($part1); }
	if (substr($part2, 0, 2) == "\\\\") { $part2 = "\\\\" . stripDirectory($part2); }
	else { $part2 = stripDirectory($part2); }
...
// Check if Unix or Windows style directories are used
	if     ($part1_len > 1 && (substr($part1, 1, 1) == ":" || substr($part1, 0, 2) == "\\\\")) { $system = "windows"; }
	elseif ($part2_len > 1 && (substr($part2, 1, 1) == ":" || substr($part1, 0, 2) == "\\\\")) { $system = "windows"; }
	else                                                                                       { $system = "unix"; }
...
// Check if the parent directory $dir is a directory
	if (!is_dir($dir) || filetype($dir) != "dir") { 
		$errormessage = __("Unable to create a temporary directory because (unvalid parent directory)");
		setErrorVars(false, $errormessage, debug_backtrace(), __FILE__, __LINE__);
		return false;
	}
...
// Check if the parent directory is writeable
	if (!is_writable($dir)){ 
		$errormessage = __("Unable to create a temporary directory because (parent directory is not writeable)");
		setErrorVars(false, $errormessage, debug_backtrace(), __FILE__, __LINE__);
		return false;
	}

// Check if the parent directory name ends with a /
	if ($dir[strlen($dir) - 1] == '/') { $trailing_slash = ""; }
	else { $trailing_slash = "/"; }
...
// Return result
	if ($result == true) { return $filename; }
	else {
		$errormessage = __("Unable to create a temporary directory (too many tries)");
		setErrorVars(false, $errormessage, debug_backtrace(), __FILE__, __LINE__);
		return false;		
	}
...
// -------------------------------------------------------------------------
	...
	if (strpos($filename, ".zip")) { $content_disposition = "inline"; }
