...
<head>
    ...
    <title>OPcache Statistik<?php echo $opcache->getData('version', 'host'); ?></title>
...
<header>
    <nav>
        <ul>
            <li><a data-for="overview" href="#overview" class="active">Übersicht</a></li>
            <?php if ($opcache->getOption('allow_filelist')): ?>
            <li><a data-for="files" href="#files">Dateien</a></li>
            <?php endif; ?>
            <?php if ($opcache->getOption('allow_reset')): ?>
            <li><a href="?reset=1" id="resetCache" onclick="return confirm('Are you sure you want to reset the cache?');">OPcache leeren</a></li>
            <?php endif; ?>
            <?php if ($opcache->getOption('allow_realtime')): ?>
            <li><a href="#" id="toggleRealtime">Aktiviere Live Update der Statistik</a></li>
            <?php endif; ?>
        </ul>
    </nav>
</header>
...
<div id="tabs">
        ...
        <div class="container">
            ...
            <div id="info">
                ...
                <div id="functions">
                    <table>
                        <thead>
                            <tr><th>Verfügbare Funktionen</th></tr>
                        </thead>
                        <tbody>
                            <?php foreach ($opcache->getData('functions') as $func): ?>
                            <tr><td><a href="http://php.net/manual/de/book.opcache.php<?php echo $func; ?>" title="OPcache Dokumentation" target="_blank"><?php echo $func; ?></a></td></tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
    ...
    <div id="files">
        <?php if ($opcache->getOption('allow_filelist')): ?>
        <form action="#">
            <label for="frmFilter">Beginnen Sie mit der Eingabe, um den Skriptpfad zu filtern</label><br>
            ...
            var directiveNodes = this.state.data.map(function (directive) {
                ...
                return React.createElement(
                    ...
                    React.createElement(
                        ...
                        React.createElement(
                            "a",
                            { href: 'http://php.net/manual/de/opcache.configuration.php#ini.' + directive.k.replace(/_/g, '-'), target: "_blank" },
                            dShow
                        )
                    ),
                    React.createElement(
